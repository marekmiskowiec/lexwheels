...
SERIES_FALLBACK_TAGS = {'From the Vault', 'Mail-In', 'New in Mainline', 'Red Edition', 'ZAMAC'}
...
def normalize_series_metadata_fields(apps, schema_editor):
    ...
    for model in HotWheelsModel.objects.all().order_by('pk'):
        ...
        if not series and special_tag in SERIES_FALLBACK_TAGS:
            series = special_tag
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('catalog', '0009_extract_series_metadata'),
    ]

    operations = [
        migrations.RunPython(normalize_series_metadata_fields, migrations.RunPython.noop),
    ]
